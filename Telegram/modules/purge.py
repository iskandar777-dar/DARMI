...
async def purge_messages(event):
    ...
    if not await can_delete_messages(message=event):
        await event.reply("Tampaknya tidak dapat menghapus pesan tersebut")
        return

    reply_msg = await event.get_reply_message()
    if not reply_msg:
        await event.reply(
            "Balas pesan untuk memilih dari mana mulai menghapus.")
        return
    messages = []
    ...
    text = f"Berhasil dibersihkan di {time_:0.2f} Detik(s)"
