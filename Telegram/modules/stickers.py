...
def stickerid(update: Update, context: CallbackContext):
    ...
    if msg.reply_to_message and msg.reply_to_message.sticker:
        ...
        update.effective_message.reply_text(
            "Hello "
            + f"{mention_html(msg.from_user.id, msg.from_user.first_name)}"
            + ", Harap balas pesan stiker untuk mendapatkan stiker id",
            parse_mode=ParseMode.HTML,
        )
...
def kang(update: Update, context: CallbackContext):  # sourcery no-metrics
    ...
    if not msg.reply_to_message and not args:
        ...
        if not packs:
            packs = "Sepertinya Anda tidak punya paket! Harap balas stiker, atau gambar ke kang dan buat paket baru!"
        else:
            packs = "Balas stiker, atau gambar ke kang it!\nNgomong-ngomong, ini paket kamu:\n" + packs
    ...
    if rep := msg.reply_to_message:
        if rep.sticker:
            ...
            msg.reply_text("Ya, aku tidak bisa mencurinya.")
            return
        ...
        try:
            ...
            msg.reply_text("Ya, itu bukan URL yang bisa saya unduh.")
            return
        except HTTPError as e:
            # if we're not allowed there for some reason
            msg.reply_text(f"Kesalahan mengunduh file: {e.code} {e.msg}")
            return
    ...
    if not is_animated and not is_video:
        ...
        try:
            ...
            msg.reply_text("Saya hanya bisa mencuri gambar m8.")
            return
    ...
    try:
        ...
        if e.message == "Stickerset_invalid":
            ...
            msg.reply_text("Ukuran kemasan maksimum tercapai. Tekan F untuk membayar hormat.")
        elif e.message == "Emoji stiker tidak valid":
            msg.reply_text("Aku tidak bisa kang dengan emoji itu!")
        elif e.message == "Sticker_video_nowebm":
            msg.reply_text(
                "Format media ini tidak didukung, saya membutuhkannya dalam format webm, "
                "[see this guide](https://core.telegram.org/stickers/webm-vp9-encoding).",
                parse_mode=ParseMode.MARKDOWN,
                disable_web_page_preview = True,
            )
        elif e.message == "Galat Server Internal: kumpulan stiker tidak ditemukan (500)":
            msg.reply_text(
                f"Stiker berhasil ditambahkan ke [pack](t.me/addstickers/{packname})\n"
                + f"Emoji adalah: {sticker_emoji}", parse_mode=ParseMode.MARKDOWN
            )
        else:
            msg.reply_text(f"Ups! sepertinya terjadi sesuatu yang seharusnya tidak terjadi! ({e.message})")
            raise
...
def makepack_internal(
    ...
    try:
        ...
        if e.message == 'Nama kumpulan stiker sudah terisi':
            ...
            return
        elif e.message in ('Peer_id_invalid', 'bot diblokir oleh pengguna'):
            msg.reply_text(
            ...
            e.message
            == 'Kesalahan Server Internal: kumpulan stiker yang dibuat tidak ditemukan (500)'
        ):
            success = True
        elif e.message == 'Sticker_video_nowebm':
            msg.reply_text(
    ...
    if success:
        msg.reply_text(
            f"Paket stiker berhasil dibuat. Mendapatkan [here](t.me/addstickers/{packname})",
            parse_mode=ParseMode.MARKDOWN,
        )
    else:
