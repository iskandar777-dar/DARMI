        ...
        if len(args) >= 1:
            ...
            if ltype in LOCK_TYPES:
                ...
                if conn:
                    ...
                    text = "Mengunci {} untuk non-admin {}!".format(ltype, chat_name)
                else:
                    ...
                    text = "Mengunci {} untuk non-admin!".format(ltype)
                ...
                if conn:
                    ...
                    chat_id = conn
                    chat_name = chat.title
                    text = "Mengunci {} untuk semua non-admin {}!".format(
                        ltype, chat_name
                    )
                else:
                    if update.effective_message.chat.type == "private":
                    ...
                    chat_id = update.effective_chat.id
                    # chat_name = update.effective_message.chat.title
                    text = "Mengunci {} untuk semua non-admin!".format(ltype)
                ...
                send_message(
                    update.effective_message,
                    "Apa yang Anda coba untuk mengunci...? Coba /locktypes untuk daftar yang dapat dikunci",
                )
        else:
            send_message(update.effective_message, "Apa yang Anda coba kunci...?")

    else:
        send_message(
            update.effective_message,
            "Saya bukan administrator atau tidak memiliki cukup hak.",
        )
    ...
    if len(args) >= 1:
        ...
        if ltype in LOCK_TYPES:
            ...
            if conn:
                ...
                # chat_id = conn
                chat_name = chat.title
                text = "{} tidak terkunci untuk semua orang yang masuk {}!".format(ltype, chat_name)
            else:
                if update.effective_message.chat.type == "private":
                    send_message(
                        update.effective_message,
            ...
            return (
                ...
                "\n#UNLOCK"
                "\n<b>Admin:</b> {}"
                "\nTidak terkunci <code>{}</code>.".format(
                    html.escape(chat.title),
                    mention_html(user.id, user.first_name),
                    ltype,
                )
                    ...
                    if "ARABIC" in check:
                        try:
                            ...
                            if excp.message != "Pesan untuk dihapus tidak ditemukan":
                                log.exception("KESALAHAN dalam penguncian")
                        break
                if message.text:
            ...
            if lockable == "bots":
                ...
                for new_mem in new_members:
                    if new_mem.is_bot:
                        ...
                        send_message(
                            update.effective_message,
                            "Hanya admin yang boleh menambahkan bot di obrolan ini! Keluar dari sini.",
                        )
                        break
            else:
                try:
...
def list_locks(update, context):
    ...
    if conn:
        res = res.replace("Mengunci", "*{}*".format(chat_name))
