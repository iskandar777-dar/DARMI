...
MARKDOWN_HELP = f"""
Penurunan harga adalah alat pemformatan yang sangat kuat yang didukung oleh telegram. {dispatcher.bot.first_name} memiliki beberapa penyempurnaan, untuk memastikan \
...
Jika Anda ingin banyak tombol pada baris yang sama, gunakan :same, seperti:
<kode>[satu](buttonurl://example.com)
...
def info(update: Update, context: CallbackContext):  # sourcery no-metrics
    ...
    if user.last_name:
        text += f"\nNama keluarga : {html.escape(user.last_name)}"

    if user.username:
        text += f"\nNama belakang : @{html.escape(user.username)}"
    ...
    try:
        ...
        if spamwtc:
            text += "<b>\n\nSpamWatch:\n</b>"
            text += "<b>Orang ini dilarang di Spamwatch!</b>"
    ...
    try:
        ...
        if user_member.status == "administrator":
            ...
            if result.custom_title:
                text += f"\nPengguna ini memegang gelar <b>{result.custom_title}</b> here."
    except BadRequest:
        pass
    ...
    if user.id == OWNER_ID:
        ...
        text += '\nOrang ini adalah bagian dari Royal'
        Nation_level_present = True
    elif user.id in SUPPORT_USERS:
        text += '\nOrang ini adalah bagian dari Sakura'
        Nation_level_present = True
    elif user.id in SARDEGNA_USERS:
        text += '\nOrang ini adalah bagian dari Sardegna'
        Nation_level_present = True
    elif user.id in WHITELIST_USERS:
        text += '\nOrang ini adalah bagian dari Neptunia'
        Nation_level_present = True
...
def markdown_help(update: Update, _):
    ...
    update.effective_message.reply_text(
        "/save tes Ini adalah tes penurunan harga. _italics_, *bold*, `code`, "
