...
def _selective_escape(to_parse: str) -> str:
    """
    Keluar dari semua penurunan harga yang tidak valid

    :param to_parse: teks untuk keluar
    :return: string penurunan harga yang valid
    """
...
def markdown_parser(
    ...
    """
    Parsing string, lolos dari semua entitas penurunan harga yang tidak valid.

    Melarikan diri dari URL untuk menghindari kerusakan URL.
    Menambahkan kembali entitas kode telegram yang diperoleh dari objek entitas.

    :param txt: teks untuk diuraikan
    :param entitas: dict entitas pesan dalam teks
    :param offset: offset pesan - panjang perintah dan nama catatan
    :return: string penurunan harga yang valid
    """
    if not entities:
        entities = {}
...
def extract_time(message, time_val):
    if any(time_val.endswith(unit) for unit in ("m", "h", "d")):
        ...
        message.reply_text(
            "Jenis waktu yang ditentukan tidak valid. Diharapkan m,h, atau d, didapat: {}".format(
                time_val[-1]
            )
        )
        return ""
