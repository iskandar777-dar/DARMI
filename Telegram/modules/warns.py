...
def warn(
    ...
    if user.id in SARDEGNA_USERS:
        if warner:
            message.reply_text("SARDEGNA tidak dapat diperingatkan.")
        else:
            message.reply_text(
                "Sardegna memicu filter peringatan otomatis!\n Saya tidak dapat memperingatkan Sardegna tetapi mereka harus menghindari penyalahgunaan ini."
            )
        return

    if user.id in WHITELIST_USERS:
        if warner:
            message.reply_text("Pengguna yang masuk daftar putih diperingatkan kebal.")
        else:
            message.reply_text(
                "Neptunus memicu filter peringatan otomatis!\nSaya tidak dapat memperingatkan pengguna Neptunus tetapi mereka harus menghindari "
                "menyalahgunakan ini. "
            )
        return
    ...
    if num_warns >= limit:
        ...
        if soft_warn:  # kick
            ...
            reply = (
                f"<code>❕</code><b>Acara Tendangan</b>\n"
                f"<code> </code><b>•  Pengguna:</b> {mention_html(user.id, user.first_name)}\n"
                f"<code> </code><b>•  Menghitung:</b> {limit}"
            )
        ...
        reply = (
            f"<code>❕</code><b>Peringatan Acara</b>\n"
            f"<code> </code><b>•  Pengguna:</b> {mention_html(user.id, user.first_name)}\n"
            f"<code> </code><b>•  Menghitung:</b> {num_warns}/{limit}"
        )
        if reason:
    ...
    if match:
        ...
        if res:
            ...
            return (
                ...
                f"<b>ID Pengguna:</b> <code>{user_member.user.id}</code>"
...
def reset_warns(update: Update, context: CallbackContext) -> str:
    ...
    if user_id:
        ...
        return (
            ...
            f"#PERINGATAN ULANG\n"
            f"<b>Admin:</b> {mention_html(user.id, user.first_name)}\n"
            f"<b>Pengguna:</b> {mention_html(warned.id, warn.first_name)}\n"
            f"<b>ID Pengguna:</b> <code>{warned.id}</code>"
        )
    else:
    ...
    if result and result[0] != 0:
        ...
        if reasons:
            ...
            update.effective_message.reply_text(
                f"Pengguna memiliki {num_warns}/{limit} peringatan, tetapi tidak ada alasan untuk peringatan tersebut."
            )
    else:
...
def add_warn_filter(update: Update, context: CallbackContext):
    ...
    update.effective_message.reply_text(f"Pengendali peringatan ditambahkan untuk '{keyword}'!")
...
def remove_warn_filter(update: Update, context: CallbackContext):
    ...
    msg.reply_text(
        "Itu bukan filter peringatan saat ini - jalankan /warning untuk semua filter peringatan aktif."
    )
    ...
    if args:
        if args[0].isdigit():
            if int(args[0]) < 3:
                ...
                msg.reply_text("Memperbarui batas peringatan menjadi {}".format(args[0]))
                return (
                    f"<b>{html.escape(chat.title)}:</b>\n"
                    f"#SET_WARN_LIMIT\n"
                    f"<b>Admin:</b> {mention_html(user.id, user.first_name)}\n"
                    f"Tetapkan batas peringatan ke <code>{args[0]}</code>"
                )
        else:
            msg.reply_text("Beri aku nomor sebagai arg!")
    else:
        limit, soft_warn = sql.get_warn_setting(chat.id)
    ...
    if args:
        if args[0].lower() in ("on", "yes"):
            ...
            return (
                f"<b>{html.escape(chat.title)}:</b>\n"
                f"<b>Admin:</b> {mention_html(user.id, user.first_name)}\n"
                f"Telah mengaktifkan peringatan kuat. Pengguna akan dilarang"
            )
            ...
            return (
                f"<b>{html.escape(chat.title)}:</b>\n"
                f"<b>Admin:</b> {mention_html(user.id, user.first_name)}\n"
                f"Telah menonaktifkan larangan. Saya hanya akan menendang pengguna."
            )
        ...
        if soft_warn:
            msg.reply_text(
                "Peringatan saat ini disetel ke *kick* pengguna saat mereka melebihi batas.",
...
def __chat_settings__(chat_id, user_id):
    ...
    return (
        f"Obrolan ini memiliki `{num_warn_filters}` memperingatkan filter. "
