...
def lookup(update, context):
    ...
    if response is True:
        ...
        stats = '**◢ succeIntellivoid• Info Perlindungan Spam**:\n'
        stats += f' • **Diperbarui pada**: `{datetime.datetime.fromtimestamp(date).strftime("%Y-%m-%d %I:%M:%S %p")}`\n'
        stats += (
            f" • **Info Obrolan**: [Link](t.me/SpamProtectionBot/?start=00_{user_id})\n"
        )
        ...
        message.reply_text("`tidak dapat mencapai SpamProtection API`", parse_mode=telegram.ParseMode.MARKDOWN)
        time.sleep(3)
