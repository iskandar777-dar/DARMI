...
def import_data(update, context):
    ...
    if conn:
        ...
        if update.effective_message.chat.type == "private":
            update.effective_message.reply_text("Ini adalah perintah grup saja!")
            return ""
    ...
    if msg.reply_to_message and msg.reply_to_message.document:
        ...
        if len(data) > 1 and str(chat.id) not in data:
            msg.reply_text(
                "Ada lebih dari satu grup di file ini dan chat.id tidak sama! Bagaimana saya harus mengimpornya?",
            )
            return
        ...
        try:
            ...
            return msg.reply_text("Terjadi masalah saat mengimpor data!")
        ...
        try:
            ...
            msg.reply_text(
                f"Terjadi kesalahan saat memulihkan data Anda. Prosesnya gagal. Jika Anda mengalami masalah dengan ini, silakan bawa ke @medsupportt",
            )

            LOGGER.exception(
                "Impt untuk obrolan %s dengan nama %s gagal.",
                str(chat.id),
                str(chat.title),
            )
            return
        ...
        if conn:
            ...
            text = "Cadangan sepenuhnya dipulihkan *{}*.".format(chat_name)
        else:
...
def export_data(update, context):  # sourcery no-metrics
    ...
    if checkchat.get("status"):
        if jam <= int(checkchat.get("value")):
            ...
            update.effective_message.reply_text(
                "Anda hanya dapat mencadangkan sekali sehari!\nAnda dapat mencadangkan lagi dalam waktu sekitar `{}`".format(
                    timeformatt,
                ),
    ...
    context.bot.sendDocument(
        ...
        caption="*Berhasil Mengekspor cadangan :*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: `KigyoRobot-Backup` ini dibuat khusus untuk catatan.".format(
