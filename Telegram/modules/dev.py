...
def leave(update: Update, context: CallbackContext):
    ...
    if args:
        ...
        try:
            ...
            update.effective_message.reply_text("Gagal keluar dari obrolan karena beberapa alasan.")
    else:
        chat = update.effective_chat
        # user = update.effective_user
        kb = [[
            InlineKeyboardButton(text="Saya yakin dengan tindakan ini.", callback_data="leavechat_cb_({})".format(chat.id))
        ]]
        update.effective_message.reply_text("Saya akan keluar dari {}, tekan tombol di bawah untuk mengonfirmasi".format(chat.title), reply_markup=InlineKeyboardMarkup(kb))
...
def leave_cb(update: Update, context: CallbackContext):
    ...
    if callback.from_user.id not in DEV_USERS:
        callback.answer(text="Ini bukan untukmu", show_alert=True)
        return
...
def pip_install(update: Update, context: CallbackContext):
    ...
    args = context.args
    if not args:
        message.reply_text("Masukkan nama paket.")
        return
    if len(args) >= 1:
...
def get_chat_by_id(update: Update, context: CallbackContext):
    ...
    args = context.args
    if not args:
        msg.reply_text("<i>Diperlukan ID obrolan</i>", parse_mode=ParseMode.HTML)
        return
    if len(args) >= 1:
        data = context.bot.get_chat(args[0])
        m = "<b>Ditemukan obrolan, di bawah ini detailnya.</b>\n\n"
