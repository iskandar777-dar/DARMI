...
def time_formatter(milliseconds: int) -> str:
    ...
    tmp = (
        ((str(days) + " hari(s), ") if days else "")
        + ((str(hours) + " jam(s), ") if hours else "")
        + ((str(minutes) + " menit(s), ") if minutes else "")
        + ((str(seconds) + " detik(s), ") if seconds else "")
        + ((str(milliseconds) + " millidetik(s), ") if milliseconds else "")
    )
    return tmp[:-2]
...
async def fetch_audio(client, message):
    ...
    if not message.reply_to_message:
        await message.reply("`Balas ke Video / Suara.`")
        return
    warner_stark = message.reply_to_message
    if warner_stark.audio is None and warner_stark.video is None:
        await message.reply("`Format Tidak Support`")
        return
    if warner_stark.video:
        lel = await message.reply("`Video Terdeteksi, Mengkonvert ke suara !`")
        warner_bros = await message.reply_to_message.download()
        stark_cmd = f"ffmpeg -i {warner_bros} -map 0:a friday.mp3"
        await runcmd(stark_cmd)
        final_warner = "friday.mp3"
    elif warner_stark.audio:
        lel = await edit_or_reply(message, "`Memulai Download !`")
