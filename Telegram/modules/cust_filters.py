...
def filters(update, context):  # sourcery no-metrics
    ...
    if not msg.reply_to_message and len(args) < 2:
        send_message(
            update.effective_message,
            "Berikan kata kunci keyboard agar filter ini dapat membalas!",
        )
        return
    ...
    if msg.reply_to_message:
        if len(args) < 2:
            send_message(
                update.effective_message,
                "Berikan kata kunci untuk filter ini untuk dibalas!",
            )
            return
        else:
            keyword = args[1]
    ...
    if not msg.reply_to_message and len(extracted) >= 2:
        ...
        if not text:
            send_message(
                update.effective_message,
                "Tidak ada pesan filter - Anda tidak bisa HANYA memiliki tombol, Anda memerlukan pesan untuk menyertainya!",
            )
            return
        ...
        send_message(
            update.effective_message,
            "Berikan kata kunci untuk balasan filter ini!",
        )
        return
        ...
        send_message(update.effective_message, "Penyaring tidak valid!")
        return
                    ...
                    try:
                        ...
                        if error_catch == "noreply":
                            try:
                                ...
                                log.exception("Kesalahan dalam filter: " + excp.message)
                                send_message(
                                    update.effective_message,
                ...
                try:
                    ...
                    if excp.message == "Unsupported url protocol":
                        try:
                            ...
                            log.exception("Pesan balasan tidak ditemukan: " + excp.message)
...
def rmall_filters(update, _):
    ...
    if member.status != "creator" and user.id not in SUDO_USERS:
        ...
        update.effective_message.reply_text(
            f"Yakin ingin menghentikan SEMUA filter {chat.title}? Tindakan ini tidak bisa dibatalkan.",
...
def get_exception(excp, filt, chat):
    if excp.message == "Unsupported url protocol":
        return "Anda tampaknya mencoba menggunakan protokol URL yang tidak didukung. Telegram tidak mendukung kunci untuk banyak protokol, seperti tg: //. Silakan coba lagi!"
    elif excp.message == "Reply message not found":
        return "tak ada jawaban"
    else:
        log.warning("Pesan %s tidak dapat diuraikan", str(filt.reply))
        log.exception(
...
def addnew_filter(update, chat_id, keyword, text, file_type, file_id, buttons):
    ...
    if len(totalfilt) >= 1000:  # Idk why i made this like function....
        msg.reply_text("Grup ini telah mencapai batas maksimal filternya, yaitu 150.")
        return False
    else:
...
def __stats__():
    return "• {} filter, di {} obrolan.".format(sql.num_filters(), sql.num_chats())
